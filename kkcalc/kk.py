...
def KK_General_PP(
    Eval_Energy, Energy, imaginary_spectrum, orders, relativistic_correction
):
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        "Calculate Kramers-Kronig transform using general piecewise-polynomial algorithm"
    )
    # Need to build x-E-n arrays
    X = numpy.tile(
        Energy[:, numpy.newaxis, numpy.newaxis], (1, len(Eval_Energy), len(orders))
    )
    E = numpy.tile(
        Eval_Energy[numpy.newaxis, :, numpy.newaxis], (len(Energy) - 1, 1, len(orders))
    )
    C = numpy.tile(imaginary_spectrum[:, numpy.newaxis, :], (1, len(Eval_Energy), 1))
    N = numpy.tile(
        orders[numpy.newaxis, numpy.newaxis, :], (len(Energy) - 1, len(Eval_Energy), 1)
    )
    poles = numpy.equal(
        X,
        numpy.tile(
            Eval_Energy[numpy.newaxis, :, numpy.newaxis], (len(Energy), 1, len(orders))
        ),
    )
    ...
    Integral = numpy.sum(
        -C
        * (-E) ** N
        * numpy.log(numpy.absolute((X[1:, :, :] + E) / (X[:-1, :, :] + E)))
        - C
        * E**N
        * (1 - poles[1:, :, :])
        * numpy.log(
            numpy.absolute(
                ...
                / (
                    (1 - poles[:-1, :, :]) * X[:-1, :, :]
                    + poles[:-1, :, :] * X[[0] + list(range(len(Energy) - 2)), :, :]
                    - E
                )
            )
        ),
    ...
    if numpy.any(orders <= -2):  # N<=-2, ln(x) terms
        ...
        Integral += numpy.sum(
            ...
            * ((-E[i]) ** N[i] + E[i] ** N[i])
            * numpy.log(
                numpy.absolute((X[1:, :, orders <= -2]) / (X[:-1, :, orders <= -2]))
            ),
            axis=(0, 2),
        )
...
def KK_PP(Eval_Energy, Energy, imaginary_spectrum, relativistic_correction):
    ...
    logger = logging.getLogger(__name__)
    logger.info(
        "Calculate Kramers-Kronig transform using (n from 1 to -3) piecewise-polynomial algorithm"
    )
    X1 = Energy[0:-1]
    X2 = Energy[1:]
    ...
    Symb_1 = (
        (Full_coeffs[0, :] * E + Full_coeffs[1, :]) * (X2 - X1)
        + 0.5 * Full_coeffs[0, :] * (X2**2 - X1**2)
        - (Full_coeffs[3, :] / E + Full_coeffs[4, :] * E**-2)
        * numpy.log(numpy.absolute(X2 / X1))
        + Full_coeffs[4, :] / E * (X2**-1 - X1**-1)
    )
    Symb_2 = (
        (-Full_coeffs[0, :] * E + Full_coeffs[1, :]) * (X2 - X1)
        + 0.5 * Full_coeffs[0, :] * (X2**2 - X1**2)
    ...
    Symb_3 = (
        ...
        * numpy.log(numpy.absolute((X2 - E + 1 * (X2 == E)) / (X1 - E + 1 * (X1 == E))))
    )
    Symb_B = numpy.sum(
        Symb_2 - Symb_1 - Symb_3, axis=1
    )  # Sum areas for approximate integral
    # Patch singularities
    hits = Energy[1:-1] == E[:, 0:-1]
    E_hits = numpy.append(
        numpy.insert(numpy.any(hits, axis=0), [0, 0], False), [False, False]
    )
    ...
    Symb_singularities[Eval_hits] = (
        C2[0, :] * XE**2
        + C2[1, :] * XE
        + C2[2, :]
        + C2[3, :] * XE**-1
        + C2[4, :] * XE**-2
    ) * numpy.log(numpy.absolute((X2 - XE) / (X1 - XE)))
    # Finish things off
...
def improve_accuracy(
    ...
    while count < recursion and numpy.sum(improved) > 0:
        # get E_midpoints
        midpoints = (E_values[new_points - 1] + E_values[new_points]) / 2.0
        # evaluate at new points
        Im_midpoints = data.coeffs_to_ASF(midpoints, temp_Im_spectrum)
        Re_midpoints = KK_PP(
            midpoints, Full_E, Imaginary_Spectrum, relativistic_correction
        )
        # evaluate error levels
        Im_error = abs(
            (Im_values[new_points - 1] + Im_values[new_points]) / 2.0 - Im_midpoints
        )
        Re_error = abs(
            (Re_values[new_points - 1] + Re_values[new_points]) / 2.0 - Re_midpoints
        )
        improved = (Im_error > tolerance) | (Re_error > tolerance)
        logger.debug(
            str(numpy.sum(improved))
            + " points (out of "
            + str(len(improved))
            + ") can be improved in pass number "
            + str(count + 1)
            + "."
        )
        total_improved_points += numpy.sum(improved)
        # insert new points and values
        Im_values = numpy.insert(
            Im_values, new_points[improved], Im_midpoints[improved]
        )
        Re_values = numpy.insert(
            Re_values, new_points[improved], Re_midpoints[improved]
        )
        ...
        new_points = numpy.where(
            numpy.insert(
                numpy.zeros(Im_values.shape, dtype=bool), new_points[improved], True
            )
        )[0]
    ...
    # pylab.show()
    logger.info(
        "Improved data accuracy by inserting "
        + str(total_improved_points)
        + " extra points."
    )
...
def kk_calculate_real(
    ...
    Parameters
    ----------
    NearEdgeDataSource : string | numpy array
            Path to file containg near-edge data, or numpy array containing the data.
    ...
    if isinstance(NearEdgeDataSource, str):
        NearEdge_Data = data.convert_data(
            data.load_data(NearEdgeDataSource, load_options),
...
if __name__ == "__main__":
    ...
    Output = kk_calculate_real(
        "../../data/LaAlO3/LaAlO3_Exp.csv",
        "LaAlO3",
